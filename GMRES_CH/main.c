...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  while((k*dt)<0.25){
    ...
    MVmulti(nrows,ia,ja,a,x_snp,q1);
	...
	d_r                    = sqrt(h[position_h][iteration]*h[position_h][iteration]+
				      h[position_h+1][iteration]*h[position_h+1][iteration]);
	...
	Axpy(nrows,ym[i],1.0,1,1,&v[position_v_out],x_snp);
      }
      MVmulti(nrows,ia,ja,a,x_snp,q1);
      Axpy(nrows,1.0,1.0,1,-1,rh,q1);
      DotProduct(nrows,q1,q1,&residual);
